...
export const DataProvider = ({ children }: DataProviderProps) => {
  ...
  function sortTopic(topicIndex: number) {
    const newData = [...data];
    const events = newData[topicIndex].events;

    events.sort((a, b) => {
      if (!a.when || !a.where) return 1;
      if (!b.when || !b.where) return -1;
  ...
  function sortAllTopics() {
    const newData = [...data];
    for (let topicIndex = 0; topicIndex < newData.length; topicIndex++) {
      const events = newData[topicIndex].events;
      events.sort((a, b) => {
        if (!a.when || !a.where) return 1;
        if (!b.when || !b.where) return -1;
        ...
        return (
          +makeDate(a.when) - +makeDate(b.when) ||
  ...
  function removeEvent(topicIndex: number, eventIndex: number) {
    const newData = [...data];
    if (eventIndex > 0) {
      newData[topicIndex].events.splice(eventIndex, 1);
    }
